...
// Start a trial by showing the dots and starting the timer
function startTrial() {
...
function drawDots(count) {
    for (let i = 0; i < count; i++) {
        ...
        // Uncomment to randomise the shape of the dot
        // dot.style.borderRadius = `${25 * Math.floor(4 * Math.random())}%`;
...
// Ensure the min and max dots are valid
minDotsInput.addEventListener('change', () => {
    const minDots = parseInt(minDotsInput.value);
    const currentMaxDots = parseInt(maxDotsInput.value);
    
    // Ensure minDots is less than maxDots by changing maxDots if necessary
    if (currentMaxDots <= minDots) {
        maxDotsInput.value = minDots + 1;
    }
});

maxDotsInput.addEventListener('change', () => {
    const maxDots = parseInt(maxDotsInput.value);
    const currentMinDots = parseInt(minDotsInput.value);
    
    // Ensure maxDots is greater than minDots by changing minDots if necessary
    if (maxDots <= currentMinDots) {
        minDotsInput.value = maxDots - 1;
    }
});